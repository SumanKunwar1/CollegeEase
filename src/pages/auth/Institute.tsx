...
export function InstituteDashboard() {
  ...
  // Find the first college as the default (or select based on other logic)
  const college = collegesData[0]; // Assuming you always want the first college
        ...
        <nav className="flex space-x-8">
          {navigation.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                to={`/dashboard/institute/${college.id}/${item.href}`}
                className={`
