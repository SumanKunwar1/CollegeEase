import {
  ...
  Plus,
  Trash,
  UploadCloud, // Added UploadCloud icon for the upload indicator
} from "lucide-react";
...
function MentorDashboard() {
  ...
  const [expertise, setExpertise] = useState<{ skill: string }[]>(
    mentorData.expertise
  );
  const [mentorStyle, setMentorStyle] = useState<{ style: string }[]>(
    mentorData.mentorStyle
  );
  const [imageUrl, setImageUrl] = useState<string>(mentorData.imageUrl);

  const handleAddExpertise = () => {
    setExpertise([...expertise, { skill: "New Skill" }]);
  };
  ...
  const handleAddMentorStyle = () => {
    setMentorStyle([...mentorStyle, { style: "New Style" }]);
  };

  const handleRemoveMentorStyle = (index: number) => {
    const newMentorStyle = mentorStyle.filter((_, i) => i !== index);
    setMentorStyle(newMentorStyle);
  };
